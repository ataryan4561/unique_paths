...
entity unique_paths_AXILiteS_s_axi is
generic (
...
end entity unique_paths_AXILiteS_s_axi;
...
architecture behave of unique_paths_AXILiteS_s_axi is
    ...
    constant ADDR_ANS_DATA_0 : INTEGER := 16#20#;
    constant ADDR_ANS_CTRL   : INTEGER := 16#24#;
    ...
    signal int_ans             : UNSIGNED(31 downto 0) := (others => '0');
    signal int_ans_ap_vld      : STD_LOGIC;
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_ANS_DATA_0 =>
                        rdata_data <= RESIZE(int_ans(31 downto 0), 32);
                    when ADDR_ANS_CTRL =>
                        rdata_data <= (0 => int_ans_ap_vld, others => '0');
                    when others =>
                        rdata_data <= (others => '0');
                    end case;
                end if;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                ...
                if (ans_ap_vld = '1') then
                    int_ans <= UNSIGNED(ans); -- clear on read
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                ...
                if (ans_ap_vld = '1') then
                    int_ans_ap_vld <= '1';
                elsif (ar_hs = '1' and raddr = ADDR_ANS_CTRL) then
                    int_ans_ap_vld <= '0'; -- clear on read
                end if;
            end if;
        end if;
    end process;
