...
typedef struct {
    ...
    XUnique_paths_uio_map maps[ MAX_UIO_MAPS ];
} XUnique_paths_uio_info;

/***************** Variable Definitions **************************************/
static XUnique_paths_uio_info uio_info;
...
static int uio_info_read_name(XUnique_paths_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XUnique_paths_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XUnique_paths_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XUnique_paths_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XUnique_paths_Initialize(XUnique_paths *InstancePtr, const char* InstanceName) {
	XUnique_paths_uio_info *InfoPtr = &uio_info;
